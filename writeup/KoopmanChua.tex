...
The period-doubled regime is particularly instructive because its spectral picture reflects the bifurcation directly. The dominant oscillatory content is no longer just the original fundamental frequency $f_0$ inherited from the limit cycle; instead, the leading conjugate pair sits near $f_0/2$, the subharmonic that is the hallmark of period doubling. The script ranks bounded modes by their contribution to the $x$-state energy, taking conjugate pairs together so that real-valued reconstructions are preserved, and selects the top three and top ten such modes for comparison.
...
\begin{figure}[H]
    ...
    \caption{Koopman mode reconstruction of the period-doubled regime. Each row compares the measured state (black) with the unit-circle-projected reconstruction (dashed) for $x$, $y$, and $z$, using all bounded modes, the top three modes, and the top ten modes. The top-ten reconstruction recovers the doubled orbit faithfully; the top-three reconstruction captures the subharmonic but loses higher harmonics that shape the loop. (Placeholder figure; produced by \texttt{run\_chua.m}.)}
