...
To interface the analog circuit with the microcontroller, additional signal-conditioning stages were required. Summing amplifiers and diode clamps were used to shift and limit the measured voltages so that they lay within the readable range of the Arduino ADC. This step was necessary because the native circuit voltages are bipolar, while the Arduino input expects a bounded unipolar voltage range. After conditioning, the Arduino sampled three measured signals, denoted here by $x$, $y$, and $z$, corresponding to the circuit state coordinates used throughout the analysis. The implementation of Chua's circuit with easy-to-find component values results in very fast dynamics. To overcome the issue of Arduino's analogRead() function and ADC taking too long due to overhead, we read directly from the R4's registers. For each regime, $5000$ samples were recorded at a sampling frequency of roughly $30$kHz.  :
...
To quantify the effect of dictionary choice, we performed a sweep over dictionary families and sizes and measured the relative one-step error in lifted space together with computation time. Five dictionary families were compared: linear (the raw state, no constant), piecewise-linear (the seven Chua-matched observables built from the kink terms), polynomial (monomials up to a chosen degree), radial basis function with $k$-means centers, and ``full'' (polynomial combined with optional RBF and PWL terms). For each regime the data were min-max normalized to $[-1,1]$ before lifting, so that high-degree monomials remain $O(1)$ and the Gram matrix stays well conditioned.
...
Beyond reading off eigenvalues, the Koopman decomposition can be used to reconstruct the measured state directly. Using the eigenvalue/eigenvector pairs $(\lambda_k, \phi_k)$ of the EDMD matrix $K$, the lifted dynamics admit the modal expansion
\begin{equation}
    \psi(x_n) \approx \sum_k a_k \,\lambda_k^n \,\phi_k,
    \qquad a_k = (\Phi^{+} \psi(x_0))_k,
\end{equation}
and projecting back to physical state coordinates with the selector matrix $C$ that picks out $x_1, x_2, x_3$ from the dictionary gives
\begin{equation}
    x_n \approx \sum_k a_k \,\lambda_k^n \,(C \phi_k).
\end{equation}
The vectors $v_k = C \phi_k \in \mathbb{R}^3$ are the Koopman \emph{modes}: they record how strongly each of the three measured states participates in the $k$-th eigenfunction. The scalar amplitude $a_k \lambda_k^n$ then drives that mode in time.
