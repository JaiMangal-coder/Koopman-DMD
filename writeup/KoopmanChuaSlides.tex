...
\begin{frame}{Chua's Circuit: Dynamical Characteristics}
    \begin{columns}[T,onlytextwidth]
        ...
        \begin{column}{0.46\textwidth}
            ...
            \vspace{0.2cm}
            \begin{block}{Characteristic frequency}
                \(\approx 500\text{--}700\,\mathrm{Hz}\)\\
                EDMD eigenfunction: \(f_k \approx 605\,\mathrm{Hz}\)
            \end{block}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Lyapunov Spectrum Across All Regimes}
    \begin{columns}[T,onlytextwidth]
        ...
        \begin{column}{0.42\textwidth}
            ...
            \vspace{0.2cm}
            Computed via Sano--Sawada local linear maps with QR
            re-orthogonalisation; Theiler window set to
            \(2\times\) first autocorrelation zero-crossing
            to exclude temporal correlations.

            \vspace{0.1cm}
            \begin{alertblock}{Double scroll}
                \([+0.084,\;-0.008,\;-1.223]\)/sample\\
                \(\Rightarrow D_{\mathrm{KY}} = 2.062\)
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}
