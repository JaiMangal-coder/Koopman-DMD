...
\begin{frame}{Chua's Circuit: Dynamical Characteristics}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.52\textwidth}
            \textbf{Route to chaos} (resistance \(R\) via potentiometer):
            \[
                \text{fixed point}
                \;\to\; \text{limit cycle}
                \;\to\; \text{period doubling}
                \;\to\; \text{chaos}
            \]
            ...
            \[
                ...
                \lambda_2 \approx 0,\quad
                \lambda_3 \lesssim -2.4
            \]

            \textbf{Kaplan--Yorke dimension:}
            \[
                D_{\mathrm{KY}}
                = 2 + \frac{\lambda_1}{|\lambda_3|}
                \approx 2.07
            \]
            Fractal attractor — non-integer dimension confirms chaos.

            \vspace{0.2cm}
            \textbf{Characteristic frequency:}
            \(\approx 500\text{--}700\,\mathrm{Hz}\)
            (EDMD eigenfunction identifies \(f_k \approx 605\,\mathrm{Hz}\))
        \end{column}
        \begin{column}{0.44\textwidth}
            ...
            \begin{alertblock}{Key point}
                Sensitive dependence means trajectories diverge on
                timescale \(\sim 1/\lambda_1\); no long-range
                prediction is possible in state space.
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}
