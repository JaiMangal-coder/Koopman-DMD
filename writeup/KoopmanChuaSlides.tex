    ...
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.48\textwidth}
            ...
            \begin{itemize}
                ...
                \item Modes: motion as simpler components
                \item Spectra: compact dynamical summaries
            \end{itemize}
        \end{column}
        \begin{column}{0.48\textwidth}
            \textbf{Nonlinear systems}
            \begin{itemize}
                \item Fixed points, cycles, bifurcations, chaos
    ...
    \begin{columns}[T,onlytextwidth]
        ...
        \begin{column}{0.46\textwidth}
            \begin{block}{Data collection}
                \begin{itemize}
                    \item Four regimes
                    \item \(5000\) samples each
                    \item \(\approx 30\,\mathrm{kHz}\)
                    \item Snapshot pairs \((x_k,x_{k+1})\)
                \end{itemize}
            \end{block}
           
            \vspace{0.2cm}
            \begin{block}{Interpretation}
                Positive \(\lambda_1\) and non-integer dimension support the double scroll as a strange attractor.
            \end{block}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Lyapunov Spectrum Across All Regimes}
    \begin{columns}[T,onlytextwidth]
        ...
        \begin{column}{0.42\textwidth}
            ...
            \begin{itemize}
                \item \(\lambda_1 < 0\): fixed point
                \item \(\lambda_1 \approx 0\): periodic orbit
                \item Period-doubled: still periodic
                \item \(\lambda_1 > 0\): chaos
            \end{itemize}
    ...
    \vspace{0.1cm}
        \begin{alertblock}{Double scroll}
            \([\lambda_1,\lambda_2,\lambda_3]
              = [+0.084,\;-0.008,\;-1.223]\)/sample.
            Positive largest exponent means nearby trajectories separate exponentially.
        \end{alertblock}
\end{frame}
